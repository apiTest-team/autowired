import { AutowiredBootOptions, bindContainer, getClassMetadata, listPreloadModule } from "../decorator";
import { ALL, APPLICATION_BOOT_KEY, CONFIG_KEY } from "../constant";
...
export class AutowiredApplication {
  ...
  // 应用上下文
  private applicationContext: Partial<IAutowiredContainer> = {};
  // 配置信息
  private readonly globalOptions: AutowiredBootOptions;
  /**
   * 构造函数
   * @param target
   */
  constructor(private readonly target: any) {
    // 获取配置信息
    this.globalOptions = (getClassMetadata(APPLICATION_BOOT_KEY, this.target) || {}) as AutowiredBootOptions;
  }
   ...
   * @private
   */
  private async prepareApplicationContext(): Promise<IAutowiredContainer> {
    // 应用参数
    const globalOptions = this.globalOptions;

    // 打印日志
    debug(`[autowired]: start "prepareApplicationContext"`);
    debug(`[autowired]: bootstrap options=${util.inspect(globalOptions)}`);
    ...
    // 创建上下文
    const applicationContext = new AutowiredContainer();
    // 绑定容器
    bindContainer(applicationContext);
    global["AUTOWIRED_APPLICATION_CONTEXT"] = applicationContext;
  ...
  static async run(target: any, args: string[]) {
    await new AutowiredApplication(target).run(args);
  }
}
