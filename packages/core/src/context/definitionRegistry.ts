  ...
  implements IIdentifierRelationShip
{
  ...
  saveClassRelation(module: any, namespace?: string) {
    const componentId = getProvideUUID(module);
    // save uuid
    ...
    if (componentId) {
      ...
      this.storeMap.set(getProvideName(module), componentId);
      // save namespace alias
      if (namespace) {
        this.storeMap.set(namespace + ':' + getProvideName(module), componentId);
      }
    }
  }
