...
import { UseWrongMethodException } from "../exception";
  ...
  async doConstructAsync(Clazz: any, args?: any): Promise<any> {
    ...
    if (this.definition.constructMethod) {
      ...
      if (Types.isAsyncFunction(fn)) {
        inst = await fn.apply(Clazz, args);
      } else {
        inst = fn.apply(Clazz, args);
      }
    } else {
      inst = Reflect.construct(Clazz, args);
    }
  ...
  doInit(obj: any): void {
    ...
    if (this.definition.initMethod && inst[this.definition.initMethod]) {
      if (
        ...
        throw new UseWrongMethodException(
          'context.get',
          'context.getAsync',
          this.definition.id
        );
